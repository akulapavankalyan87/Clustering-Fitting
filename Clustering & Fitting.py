...
df, df_country, df_year = read_df('wb_dataset.csv')
...
def balance_data(df):
  ...
  min_length = min(len(greenhouse_gas_emissions), len(agricultural_land), len(GDP))
...
scaler = MinMaxScaler()
transform_data = scaler.fit_transform(df[['greenhouse_gas_emissions', 'agricultural_land', 'GDP']])
...
for i in range(5):
    ...
    plt.scatter(cluster_data['greenhouse_gas_emissions'], cluster_data['GDP'], label=f'Cluster {i}')
...
for i in range(5):
    cluster_data = df[df['cluster'] == i]
    plt.scatter(cluster_data['agricultural_land'], cluster_data['GDP'], label=f'Cluster {i}')
...
nep = df[df['country'] == 'China']

# create a plot showing the clusters and cluster centers using pyplot
for i in range(5):
    cluster_data = nep[nep['cluster'] == i]
    plt.scatter(nep['greenhouse_gas_emissions'], nep['GDP'], label=f'Cluster {i}')
